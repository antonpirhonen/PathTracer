...
int main() {
    ...
    MaterialTransparent transparent = MaterialTransparent(false, Color(0,0,0), Color(1,0,1), 1.5);
...
//    Triangle trans_tr_pain = Triangle(transparent, Vec3{3,1,-1}, Vec3{3.5,1,1}, Vec3{2.5,-1,-1});
    Triangle trans_tr_suora = Triangle(diffuse, Vec3{2, 0.75, -0.75},  Vec3{5,0.75,-0.75}, Vec3{5,0.75,1.25});

    env.AddBody(&rad_tr);
//    env.AddBody(&trans_tr_pois);
//    env.AddBody(&trans_tr_pain);
    env.AddBody(&trans_tr_suora);
